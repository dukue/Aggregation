...
const SourceManager = ({ navigation }) => {
  ...
  const handleSave = async (editedSource) => {
    try {
      await sourceManager.updateSource(editedSource.source);
      await loadSources();
    ...
    <View style={styles.container}>
      ...
      <Portal>
        <FAB.Group
          ...
          actions={[
            {
              icon: ({ size, color }) => <Library size={size} color={color} />,
              label: '导入文件',
              onPress: handleImportFile,
            },
            {
              icon: ({ size, color }) => <QrCode size={size} color={color} />,
              label: '扫描二维码',
              onPress: () => {/* 处理扫描二维码 */},
            },
            {
            ...
            {
              icon: ({ size, color }) => <RefreshCcw size={size} color={color} />,
              label: '刷新',
              onPress: loadSources,
            },
            {
...
const styles = StyleSheet.create({
  ...
  fab: {
    ...
    marginBottom: Platform.select({
      ios: 80, // iOS 底部导航栏通常更高
      android: 72, // Android 底部导航栏
    }),
  },
  textInput: {
    marginTop: 8,
